...
class ElsSearch():
    ...
    def __init__(self, query, index):
        """Initializes a search object with a query and target index.
            ...
            If query is a string, we assume it is the query params for
                the GET method API.
              See
              https://dev.elsevier.com/documentation/ScienceDirectSearchAPI.wadl

            If query is a dict, we assume it defines the query params for
                the PUT method API
        ...
        self._uri = self._base_url + self.index
    ...
    def execute(self, els_client=None, get_all=False):
        """Executes the search using GET or PUT depending query type
            If get_all = False (default), this retrieves
                the default number of results specified for the API.
            If get_all = True, multiple API calls will be made to iteratively
                get all results for the search, up to a maximum of 5,000.
        """
        if type(self.query) == type(''):
            self.executeGET(els_client, get_all=get_all)
        elif type(self.query) == type({}):
            self.executePUT(els_client, get_all=get_all)
        else:
            raise TypeError('query is not a string or a dictionary')
    
    def executeGET(self, els_client = None, get_all = False):
        """Executes the search using the GET method.
            If get_all = False (default), this retrieves
        ...
        self._uri += "?query=" + url_encode(self.query)
    ...
    def executePUT(self, els_client=None, get_all=False):
        ...
        increment = 100         # the number of results to get on each API call
        if get_all is True:     # take over the display 'show' & 'offset' attrs
            query = deepcopy(self.query)
            displayField = query.get('display', {})
            displayField['show'] = increment
        ...
        api_response = els_client.exec_requestPUT(self._uri, params=queryJson)
        ...
        if get_all is True:     ## do any needed additional API calls
            while (self.num_res < self.tot_num_res) and \
                                            not self._upper_limit_reached():
                query['display']['offset'] += increment
                
                queryJson = json.dumps(query)
                api_response = els_client.exec_requestPUT(self._uri,
                                                            params=queryJson)
                self._results += api_response['results']
